...
export default class MapUtils {
    ...
    static convertCoordToTile(coord, zoom) {
        ...
        result.ytile = (Math.floor( (1 - Math.log(Math.tan(coord.lat * Math.PI / 180) + 1 
                            / Math.cos(coord.lat * Math.PI / 180)) / Math.PI) / 2 * n));
        result.zoom = zoom;
        return result;
    }
    ...
    static convertTileToCoord(tile) {
        ...
        result.lat = (180 / Math.PI * Math.atan(0.5 * (Math.exp(m) - Math.exp(-m))));
        return result;
    }
}
